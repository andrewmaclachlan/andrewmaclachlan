---
authors:
- Andrew MacLachlan
bio:
education:
  courses:
  - course: PhD in Geography and Envrionment 
    institution: The University of Southampton and The University of Western Australia
    year: 2018
  - course: MSc in GIS and Remote Sensing
    institution: The University of Southamtpon 
    year: 2014
  - course: BA in Human Geography
    institution: The University of Sheffield, with study abroad at The University of Wollongong
    year: 2013
email: "a.maclachlan@ucl.ac.uk"
interests:
- Spatial analysis
- Remote sensing
- Applied policy research 
name: Andrew MacLachlan
organizations:
- name: University College London
  url: "https://www.ucl.ac.uk/bartlett/casa/"
role: Teaching Fellow in Spatial Data Science and Visualisation 
social:
# - icon: paper-plane
# icon_pack: fas
#  link: /contact
- icon: envelope
  icon_pack: fas
  link: '#contact'
- icon: twitter
  icon_pack: fab
  link: https://twitter.com/ac_maclachlan
- icon: google-scholar
  icon_pack: ai
  link: https://scholar.google.com/citations?user=wLrJwyQAAAAJ&hl=en
- icon: github
  icon_pack: fab
  link: https://github.com/andrewmaclachlan
- icon: linkedin-in
  icon_pack: fab
  link: https://www.linkedin.com/in/amaclachlan/
superuser: true
user_groups:
- Researchers
- Visitors
---

test Andrew's research interests revolve around developing data-driven urban development recommendations to enhance future sustainability and optimise resource investment. This covers the broad themes of: city science, spatial data analysis, dynamic population modelling and earth observation. His doctoral research used Earth observation to quantify urban growth and demonstrated its application to sustainable city development. With a background in remote sensing Andrew has also been involved with field validation of Sentinel-3 data. 
